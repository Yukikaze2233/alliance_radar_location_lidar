...
public:
    ...
    auto ConfigLoader(const YAML::Node& model_config) -> std::expected<bool, std::string> {
        ...
        if (const auto map_node = model_node["map_path"]; map_node.IsDefined() && !map_node.as<std::string>().empty()) {
            map_path = map_node.as<std::string>();
        } else {
            return std::unexpected("Model.map_path is required (should point to the PCD generated by model_to_map)");
        }
    ...
    auto LoadPointCloud() -> std::expected<bool, std::string> {
        ...
        RCLCPP_INFO(rclcpp::get_logger("model_preprocess"),
            "Loaded %zu points from %s", point_cloud.size(), map_path.string().c_str());
        return true;
    }
...
ModelProcess::ModelProcess() noexcept : impl_ { std::make_unique<Impl>() } { }
