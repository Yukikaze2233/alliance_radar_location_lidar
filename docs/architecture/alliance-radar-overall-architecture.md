...
│
├── Asset / Tool Layer
│   ├── radar_model                   ← 模型 / 赛场资产 -> 地图点云资产
│   └── radar_calibration             ← 标定数据 -> 外参 YAML
│
├── Perception Layer
│   ├── radar_lidar                   ← /livox/lidar -> /lidar/pose_raw, /lidar/pose
│   └── radar_camera                  ← image stream -> /camera/pose
│
